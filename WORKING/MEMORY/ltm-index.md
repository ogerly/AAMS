...
| 125 | 2026-04-29 | ARCH | `skills` | Issue #51 konzipiert + implementiert. `skills` in `.agent.json` + AGENT.json + AGENT_SCHEMA.json + CONTRACT.md. Manifest-Prinzip (D9): beschreibend, nicht preskriptiv. Global → Lokal: Quelle → Feinschliff. Workpaper → closed/. |
 ...
 | 141 | 2026-09-16 | MKT | `WP-2026-09-16-MKT-DOC-readme-website-analyse.md` | README + Webseite Relaunch: Ist→Soll-Analyse (F1–F8, M1–M9). M1+M2 umgesetzt (Naming „Agent Manifest", DEVmatrose/AAMS→ogerly/AAMS in 13 Live-Dateien inkl. EN/ZH). Alte Landing→`docs/_archive/`, Landing bereinigt als neues `docs/index.html`. Alte README→`README.old.md`, neue `README.md` (~165 Zeilen, Status 2.4.0/WH-011). Slogan beides. Audit-Ausnahme: closed/, ltm-index, WHITEPAPER unangetastet. Follow-ups: M7 EN/ZH-Rewrite, LICENSE-Datei, Max-5. |
 | 142 | 2026-09-16 | SPEC | `WP-2026-09-16-SPEC-QUEST-hermes-sidecar-stellungnahme.md` | Issue #53 beantwortet (Kommentar 5692798068): Hermes-Sidecar Stufe 2 als Local Adaptation D9-konform, Session-Guard ≠ Write-Guard (WH-009), Upstream nur als generisches Pattern unter 5 Bedingungen, eindeutig optionales Modul, R1–R3 vertraglich eingefangen, Evidenz E1–E7. D2-Promotion resolved: belassen (kein RFC eingegangen). Commit 73103b5 gepusht. |
 | 143 | 2026-09-22 | SPEC | `WP-2026-09-22-SPEC-NOTE-hermes-namensabgrenzung-nous-vs-local.md` | Hermes-Namensabgrenzung (WP-012, offen): **Hermes-Local** = Session-/Workpaper-Guard im AAMS-Ritual (WP-2026-09-16, `descriptive_only`) ≠ **Hermes-Nous** = Nous-Research-Agent-Runtime (`state.db`, `MEMORY.md`, fremde Runtime, kein AAMS-Modul). D1: Repo-Konvention Local/Nous-Label. D2: WP-2026-09-16 unverändert. D3: AAMS ≠ Nous-Plugin. Promotion-Pflicht: D1+D3 → WH-002 vor Close. |
 | 144 | 2026-09-22 | RES | `observe/WP-2026-09-22-RES-JEV-open-jev-decision-layer.md` | Open-Jev Decision Layer (WP-013, observe): TypeSafe-JEV = closed/hosted, Cloud-Pflicht verworfen (D2). Optionsraum Open-Jev-Reimplementierungen (kyegomez, Zefan-Cai Qwen-Basis, OpenSourceJev) — **alle Claims unverifiziert** (Quelle: Chat-Antwort). Architektur-Skizze: AAMS-State → lokale Decision Layer (Decision+Wahrscheinlichkeit) → Qwen/Tools → AAMS. D1: nur lokale optionale Local Adaptation. D3: Repos vor Entscheidung verifizieren. Gleiche Abgrenzungslogik wie Hermes-Nous (143). |
 | 146 | 2026-09-22 | RES | `observe/WP-2026-09-22-RES-JEV-open-jev-decision-layer.md` (v2) | Jev-WP erweitert durch 09:30-Dump (849 Zeilen, `closed/raw-dump-*-open-jev-layer-chat-dump.md`, v1 separat archiviert): **User-Positionierung bindend: Cloud-/API-Jev ist keine Option** (Jev original = closed/hosted). Kernfrage: Decision-Layer-Pattern in AAMS-Spec (`descriptive_only`) vs. Umsetzung in **MANTIS-LLM-PROXY-3** (Proxy-Schicht). Rollenmodell: AAMS remembers → Qwen reasons → Decision-Layer decides → OpenCode acts → AAMS records. 8 Use-Cases (next_action, Qwen-Kontrolle, AAMS-Regelbasis `WORKING/JEV/`, Gatekeeper, Decision-Context-Builder, Context-Governor, Confidence-Schwellen ≥0.90/0.70/0.50, `aams-jev-gate`). UC7 (Context-Governor) = stärkste Kandidatin für AAMS-Pattern (bindet an Workpaper-Lifecycle). **D4 revidiert (User-Prinzip, siehe 147): Spec-First.** |
 | 147 | 2026-09-22 | GOV | WP-012 + WP-013 + `opencode.json` | **Spec-First-Prinzip (User, bindend):** Die Hermes-/JEV-Diskussion ist, ob die Patterns in der **AAMS-Spezifikation** Sinn machen (`descriptive_only`). „Die Spezifikation ist das Wichtigste. Kein Abweichen von diesem Pfad." **MANTIS-Proxy = selbstgebaute Brücke** zwischen LM Studio / OpenCode / VS Code (Provider `mantis`, „MANTIS LLM Gateway", OpenAI-kompatibel `/v1`, Default qwen3.8-27b lokal) — **muss immer laufen**, Infrastruktur, **kein Entscheidungsziel** (höchstens späterer Umsetzungsort). D4 von WP-013 entsprechend revidiert. |
 | 148 | 2026-09-22 | GOV | `WP-2026-09-22-GOV-GRP-aams-arbeitsgrundstruktur-gruppenprozesse.md` | **AAMS als Arbeitsgrundstruktur für autonome Gruppen** (WP-015, offen): User-Ziel = souverän + eigenständig arbeiten, **keine verlorenen Entscheidungen/Entwicklungen** in autonomen Gruppen. Argumente: Decision-Promotion + File Protocol + State Recovery (Disk=ground truth) = kein Decision-Verlust; tool-agnostisch + lokal (MANTIS-Proxy) + Guard = Souveränität ohne Lock-in; Lifecycle/Max-5/INDEX/RFL = Autonomie mit Ordnung. Mapping: Agent=Workpaper, Geteilte-Wahrheit=WHITEPAPER+MEMORY, Warten=observe/, Handover=on_session_start. Input: Claude-Artefakt ed479cb4… **BLOCKED (Login-Wall)** — User-Input ausstehend (D3). D1: AAMS = Kandidat-Arbeitsgrundstruktur, pending Artefakt-Analyse. D2: WP-004 → observe/ (Max-5). |
 | 149 | 2026-09-22 | GOV | `WP-015` §4/§4.1/§5.6 + `closed/raw-2026-09-22-claude-artifact-gruppenprozesse.md` | **Real Life Netzwerk-Modell analysiert** (Artefakt via User-Kopie): zwei Ebenen (freies offenes Netzwerk + verbindliche Kern-Organisation), Kreise (autonom/gekoppelt), **Kopplung = Menschen** („ist kein Ort. Ist ein Mensch."), Zwei-Schultern, Anwesenheit-statt-Amt, Zellteilung >7, Leitplanken (Kein Durchgriff, freier Exit, Kopplung-statt-Befehl), **Focalizer** (Verantwortung, dass es passiert). **Gap (§4.1):** Modell regelt Wer–Wer, aber nicht Persistenz (Wissen in Köpfen, Zellteilung-Erbe, Exit-Amnesie, Focalizer-Überblick, Agenten-Kopplung). **These (§5.6): „Kopplung sind Menschen. Gedächtnis muss Struktur sein."** — AAMS = Gedächtnis-Substrat des Kreis-Modells: 3. Schulter neben Zwei-Schultern, Zellteilung-Erbfolge per Whitepapers, Exit≠Amnesie, abfragbarer Focalizer-Überblick (INDEX/Diary/LTM/wiki_lint), Onboarding=on_first_entry, WORKSPACE-Discovery = Multi-Space. **Agenten-Kreise: Kopplung nur über Dateien → AAMS = Voraussetzung, keine Option.** D1 neu: AAMS als Arbeitsgrundstruktur positioniert (pending User-Bestätigung). |
 | 150 | 2026-09-22 | MKT | `docs/aams-arbeitsgrundstruktur-argumente.md` | Argumente-Exposé für das Real-Life-Netzwerk erstellt (auf User-Wunsch): These „Kopplung sind Menschen, Gedächtnis muss Struktur sein", Gap-Tabelle, 5 Argumentationsblöcke (Kein-Verlust/Souveränität/Autonomie-Ordnung/Kreis-Modell-unangetastet/Agenten=Voraussetzung), Gegenargumente+Antworten, 10-Min-Explainer, 5-Min-Probierlauf, Anker-Tabelle (WH-001/005/009, WP-015, Rohquelle). Ziel: Diskussionsgrundlage in der Gruppe (vor D1-Bestätigung). |
 | 151 | 2026-09-22 | SPEC | `WP-2026-09-22-SPEC-TEAM-agent-json-team-fähigkeit-analyse.md` (WP-016) | **AAMS-Spec team-fähig (Analyse):** User-Idee: fast alles bleibt; nur Workpaper + MEMORY/LTM + Diary bekommen die Komponente „anderer Mensch/Agent arbeitet weiter daran" (wer + womit = Tool + Modell). **Ist-Zustand:** `{agent}`-Naming + `behavioral_signals` + `metadata.team_size` existieren schon; fehlt: Roster (G1), Authorship (G2), Handover `continued_from` (G3), Modell-Kontext für Review (G4); **Drift-Funde:** reference/AGENT.json = 2.3.1 vs. .agent.json = 2.4.0 (G5) + doppelter `skills`-Key (G6) + kein `team` im Schema (G7). **Zwei Wege:** (1) optionale `team`-Sektion in Spec (Empfehlung, D1 pending) → AAMS/2.5.0, D9-descriptive, backward-kompatibel; (2) nur Konvention (Con: Spec = Distributionskanal via curl, andere Teams erreichen es nie). **D2 (User, bindend): Whitepaper bleiben author-frei** (Wahrheit-in-Zeit). D3: Authorship = worker+tool+model, Handover = `continued_from` + observe, Single-Writer. Modell-Info (Frontier vs. qwen3.8-27b) = Review-Tiefe, Focalizer-Delegation (WP-015), Handover-Vertrauen. |
 | 152 | 2026-09-22 | SPEC | `WP-016` v2 (Design final) | **Team-Authorship Design v2 FINAL (5 User-Antworten):** (1) Roster **dynamisch** → `WORKING/TEAM.md`, Bootstrap create_if_missing, **ab Tag 1 auch solo**. (2) **Volle Modell-ID + optionales capability-Tag** (local/frontier). (3) **Guard-Check `authorship_present`** vor Write (Reihenfolge: manifest_read → workpaper_open → authorship_present → tools_gated). (4) **Detection-basierte Authorship**: Tool/Modell/capability passiv aus Tech-Stack (tool_detection + Runtime-Konfiguration + Endpoint/Provider: localhost/LAN→local, Cloud→frontier); Name nur wenn bekannt, sonst `—`; **niemals an den Menschen gefragt — AAMS bleibt Spezifikation**; Beispiel: opencode.json (Provider mantis, qwen3.8-27b, Label „lokal") → `BY: — / opencode / qwen/qwen3.8-27b / local`. (5) **Kein Pilot in diesem Repo (erstmal)** — Adoption später via standardisiertem Upgrade-Pfad (curl + on_update) = WH-011-Test. **Neues Prinzip: Struktur-Invarianz — Team oder Solo = gleiche Struktur, null Unterschied, kein Team-Zweig im Schema.** D1 bestätigt, D7–D10 neu; D9-WP = Guard-Check (lokal, nicht das globale D9). Nächstes: 2.5.0-Implementierung (Spec-Dateien + Schema + CHANGELOG + bootstrap TEAM.md), Push = User-Entscheidung. |
 | 153 | 2026-09-22 | GOV | `WP-016` v3 + `github.com/ogerly/AAMS/releases` | **Release-Plan (D11, User-Auftrag):** Neues Release anlegen + beschreiben. **GitHub-Status (2026-09-22): Latest = v2.3.2** („on_update migriert Konventionen"); **v2.4.0 + v2.5.0 = nur lokale Tags, KEIN GitHub-Release** (v2.5.0-Tag zeigt auf alten Commit 6ed5003, Manifest dort 2.4.0 — G8). → **Release = v2.6.0** (nächstes freie, altes Tag unangetastet, Git-Safety). Beschreibung deckt „alles seit v2.3.2" ab: Team-Authorship + v2.4.0-Content (Upgrade-Sicherheit/-Report/UPGRADE.md) + v2.5.0-Content (INDEX-Pflege/Max-5). Stil wie bestehende Releases (## Added/Changed/Decisions/Upgrade + Get-Started-curl). Plan: Implementierung → Commit → Tag v2.6.0 → Push (User) → GitHub-Release (User/Agent+Token). Offene Entscheidungen: retroaktive v2.4.0/v2.5.0-Seiten? Push-Weg? Assets (Source vs. 3-Command-Bundle)? |
 | 145 | 2026-09-22 | GOV | `WP-2026-09-22-GOV-SYNC-dump-auflösung-status-sync.md` + `WORKING/WORKPAPER/INDEX.md` | Total-AAMS-Check: AAMS/2.4.0 stabil, kein Versions-Drift. 2 Chat-Dumps → saubere WPs (143/144), Roh-Dumps → closed/ (raw-dump-*). `skill-opencode-agent.md` reklassifiziert: Skill, kein WP → `TOOLS/skills/opencode/opencode-agent.md`. INDEX neu: 5 offen (Max-5), 4 observe, closed-Sektion korrekt. READ-AGENT Status fix (`_contract` 2.4.0, LTM 145). `.agent.json` doppelter topic_registry-Key entfernt. Offene Baustellen: WH-007 fehlt (in INDEX gelistet), 3 alte offene WPs (004/007/008), wiki_lint 12E/28W pre-existing, docs/logo* untracked. |
